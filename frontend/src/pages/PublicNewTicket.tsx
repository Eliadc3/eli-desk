...
export default function PublicNewTicket() {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const t = await createPublicTicket({ hospitalDepartmentId, subject, description, name: name || undefined, email: email || undefined, phone: phone || undefined });
      toast({ title: "Ticket submitted", description: `Ticket #${t.number ?? ""}` });
      setSubject(""); setDescription(""); setName(""); setEmail(""); setPhone("");
    } catch (e: any) {
      toast({ title: "Submit failed", description: e?.response?.data?.message ?? e?.message ?? "Unknown error", variant: "destructive" });
    } finally {
      setBusy(false);
    }
  };
